...
$mw_settings         = matrixweave()->settings;
$mw_has_secret       = '' !== $mw_settings->get_secret_key();
$mw_secret_locked    = $mw_settings->secret_is_from_constant();
$mw_public_key       = $mw_settings->get_public_key();
$mw_order_lookup_on  = $mw_settings->is_order_lookup_enabled();
$mw_embed_on         = $mw_settings->is_embed_enabled();
$mw_wc_active        = class_exists( 'WooCommerce' );
?>
<div class="wrap matrixweave-wrap">
	...
	<div class="matrixweave-status-row">
		<span class="matrixweave-pill <?php echo $mw_public_key ? 'is-on' : 'is-off'; ?>">
			<?php echo $mw_public_key ? esc_html__( 'Widget key set', 'matrixweave-for-woocommerce' ) : esc_html__( 'Widget key missing', 'matrixweave-for-woocommerce' ); ?>
		</span>
		<span class="matrixweave-pill <?php echo $mw_has_secret ? 'is-on' : 'is-off'; ?>">
			<?php echo $mw_has_secret ? esc_html__( 'Order lookups ready', 'matrixweave-for-woocommerce' ) : esc_html__( 'Order lookups off', 'matrixweave-for-woocommerce' ); ?>
		</span>
		<span class="matrixweave-pill <?php echo $mw_wc_active ? 'is-on' : 'is-off'; ?>">
			<?php echo $mw_wc_active ? esc_html__( 'WooCommerce active', 'matrixweave-for-woocommerce' ) : esc_html__( 'WooCommerce inactive', 'matrixweave-for-woocommerce' ); ?>
		</span>
	</div>
		...
		<div class="matrixweave-main">
			<form method="post" action="options.php">
				...
				<div class="matrixweave-card">
						...
						<tr>
							...
							<td>
								<input type="text" id="mw_public_key" class="regular-text code"
									...
									value="<?php echo esc_attr( $mw_public_key ); ?>"
							...
							<td>
								...
								<?php else : ?>
									<input type="password" id="mw_secret_key" class="regular-text code"
										...
										placeholder="<?php echo $mw_has_secret ? esc_attr__( '•••••••• saved — leave blank to keep', 'matrixweave-for-woocommerce' ) : 'sk_...'; ?>" />
						...
						<tr>
							...
							<td>
								<label class="matrixweave-switch">
									...
									<input type="checkbox" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[embed_widget]" value="yes" <?php checked( $mw_embed_on ); ?> />
						...
						<tr>
							...
							<td>
								<label class="matrixweave-switch">
									...
									<input type="checkbox" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[order_lookup]" value="yes" <?php checked( $mw_order_lookup_on ); ?> />
						...
						<tr>
							...
							<td>
								<select id="mw_mode" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[mode]">
									<option value="auto" <?php selected( $mw_settings->get_mode(), 'auto' ); ?>><?php esc_html_e( 'Auto (Sales + Support)', 'matrixweave-for-woocommerce' ); ?></option>
									<option value="sales_agent" <?php selected( $mw_settings->get_mode(), 'sales_agent' ); ?>><?php esc_html_e( 'Sales', 'matrixweave-for-woocommerce' ); ?></option>
									<option value="support_agent" <?php selected( $mw_settings->get_mode(), 'support_agent' ); ?>><?php esc_html_e( 'Support', 'matrixweave-for-woocommerce' ); ?></option>
								</select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<input type="text" id="mw_primary_color" class="regular-text"
									...
									value="<?php echo esc_attr( $mw_settings->get_primary_color() ); ?>" placeholder="#6366f1" />
						...
						<tr>
							...
							<td>
								<input type="text" id="mw_greeting" class="large-text"
									...
									value="<?php echo esc_attr( $mw_settings->get_greeting() ); ?>" placeholder="<?php esc_attr_e( 'Hi! How can I help?', 'matrixweave-for-woocommerce' ); ?>" />
							</td>
						</tr>
					</table>
					...
					<details class="matrixweave-advanced">
						...
						<table class="form-table" role="presentation">
							<tr>
								...
								<td><input type="url" id="mw_api_url" class="regular-text code" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[api_url]" value="<?php echo esc_attr( $mw_settings->get_api_url() ); ?>" /></td>
							</tr>
							<tr>
								<th scope="row"><label for="mw_widget_url"><?php esc_html_e( 'Widget script URL', 'matrixweave-for-woocommerce' ); ?></label></th>
								<td><input type="url" id="mw_widget_url" class="regular-text code" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[widget_url]" value="<?php echo esc_attr( $mw_settings->get_widget_url() ); ?>" /></td>
							</tr>
						</table>
					</details>
					...
					<input type="hidden" id="mw-key-write-mirror" name="<?php echo esc_attr( Matrixweave_Settings::OPTION_KEY ); ?>[key_write]" value="<?php echo esc_attr( $mw_settings->is_key_write_enabled() ? 'yes' : 'no' ); ?>" />
		...
		<div class="matrixweave-side">
			<div class="matrixweave-card matrixweave-connect">
				...
				<label class="matrixweave-switch matrixweave-write-toggle">
					<input type="checkbox" id="mw-key-write" value="1" <?php checked( $mw_settings->is_key_write_enabled() ); ?> />
					<span><?php esc_html_e( 'Allow the agent to create orders (Read/Write)', 'matrixweave-for-woocommerce' ); ?></span>
				</label>

				<button type="button" class="button button-primary" id="mw-generate-key" <?php disabled( ! $mw_wc_active ); ?>>
